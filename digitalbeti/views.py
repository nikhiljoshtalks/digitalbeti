...
from digitalbeti.forms import BeneficiaryDataForm, AddressForm, SignUpForm, VLEDataForm, VLECompetitionForm, \
    UserCreateForm, DMDCDistrictInput, BulkVLECreation
...
def vle_user_exam(request):
    ...
    if not unfinished:
        can_download_certificate = True
    print(can_download_certificate)
    if request.method == 'POST' and request.POST.get('exam_id'):
...
class UserCertificateView(WeasyTemplateView):
...
class VLECertificateView(WeasyTemplateView):
    print(settings.BASE_DIR + '/bin/vlecertificate.css')
    pdf_stylesheets = [
        settings.BASE_DIR + '/bin/vlecertificate.css',
    ]
    template_name = "digitalbeti/certificate/certificate.html"
...
@login_required
def all_vles(request):
    if request.user.digitalbetiuser.kind != 'ADMIN':
        messages.error(request, "Only admin allowed to delete user")
        return redirect(reverse('user_dashboard'))
    print(request.user.digitalbetiuser)
    Vles = DigitalBetiUser.objects.filter(kind='VLE')
    context = {
        'Vles': Vles
    }
    return render(request, 'digitalbeti/logged-in/admin/all-vles.html', context)


@login_required
def delete_vle(request, email=None):
    if request.user.digitalbetiuser.kind != 'ADMIN':
        messages.error(request, "Only admin allowed to delete user")
        return redirect(reverse('user_dashboard'))
    if request.method == 'POST':
        print(request.POST)
        Vle = User.objects.get(email=email)
        Vle.delete()
        return redirect(reverse('admin_all_vles'))
    Vle = User.objects.get(email=email)
    context = {
        'Vle': Vle
    }
    return render(request, 'digitalbeti/logged-in/admin/delete_user.html', context)
